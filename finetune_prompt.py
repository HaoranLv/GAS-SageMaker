...
from data_utils_prompt import ABSADataset
from data_utils_prompt import read_line_examples_from_file
from eval_utils_prompt import compute_scores
...
def init_args():
    ...
    parser.add_argument("--do_direct_eval", default=False,  type=lambda x: (str(x).lower() == 'true'),
                        help="Whether to run eval on the dev/test set.")
    parser.add_argument("--do_inference", default=False,  type=lambda x: (str(x).lower() == 'true'),
...
dataset = ABSADataset(tokenizer=tokenizer, data_dir=args.dataset, 
                      data_type='dev', task=args.task, max_len=args.max_seq_length)
...
if args.do_direct_eval:
    ...
    test_dataset = ABSADataset(tokenizer, data_dir=args.dataset, 
                               data_type='test',task=args.task, max_len=args.max_seq_length)
